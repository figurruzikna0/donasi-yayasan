    ...
    <div class="section">
        <table class="info-table">
            <tr>
                ...
                <td class="value">{{ $sponsorship->donor_name }}</td>
                <td class="label" style="padding-left:40px;">Order ID</td>
                <td class="value">{{ $sponsorship->order_id }}</td>
            </tr>
            <tr>
                <td class="label">Email</td>
                <td class="value">{{ $sponsorship->donor_email }}</td>
                <td class="label" style="padding-left:40px;">Tanggal</td>
                <td class="value">{{ $sponsorship->created_at ? $sponsorship->created_at->format('d M Y H:i') : '-' }}</td>
            </tr>
            <tr>
                <td class="label">{{ $sponsorship->donor_phone ? 'No. WA' : '' }}</td>
                ...
                <td>
                    @php
                        ...
                        $sClass = match(true) {
                            $sponsorship->status === 'success' && !$isExpired => 'status-success',
                            $sponsorship->status === 'pending' => 'status-pending',
