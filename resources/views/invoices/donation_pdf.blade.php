...
<body>
    ...
    <div class="section">
        <table class="info-table">
            <tr>
                ...
                <td class="value">{{ $donation->order_id }}</td>
            </tr>
            <tr>
                <td class="label">Email</td>
                <td class="value">{{ $donation->donor_email }}</td>
                <td class="label" style="padding-left:40px;">Tanggal</td>
                <td class="value">{{ $donation->created_at ? $donation->created_at->format('d M Y H:i') : '-' }}</td>
            </tr>
            <tr>
                <td class="label">{{ $donation->donor_phone ? 'No. WA' : '' }}</td>
    ...
    <table class="invoice-table">
        ...
        <tbody>
            <tr>
                <td>
                    <strong>Donasi {{ $donation->campaign?->title ?? 'Program Donasi' }}</strong><br>
                    <span style="color:#888;font-size:10px;">Metode: {{ $donation->payment_method ?? '-' }}</span>
                </td>
