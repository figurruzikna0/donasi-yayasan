            ...
            <div class="card bg-base-100 shadow-md border border-emerald-200">
                <div class="card-body flex-row items-center gap-3 p-5">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-black text-emerald-700">{{ $newsList->total() }}</div>
                    </div>
                </div>
            </div>
            <div class="card bg-base-100 shadow-md border border-emerald-200">
                <div class="card-body flex-row items-center gap-3 p-5">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-black text-emerald-700">{{ $newsList->getCollection()->where('status', 'published')->count() }}</div>
                    </div>
                </div>
            </div>
            <div class="card bg-base-100 shadow-md border border-emerald-200">
                <div class="card-body flex-row items-center gap-3 p-5">
                    ...
                    <div>
                        ...
                        <div class="text-2xl font-black text-emerald-700">{{ $newsList->getCollection()->where('status', 'draft')->count() }}</div>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        @forelse($newsList as $item)
                            <tr>
                                <td>
                                    @if($item->foto_utama)
                                        <img src="{{ asset('storage/' . $item->foto_utama) }}"
                                             alt="{{ $item->judul }}" class="w-[60px] h-11 object-cover rounded border border-emerald-200">
                                    @else
                                ...
                                <td>
                                    ...
                                    <span class="badge badge-ghost badge-sm">{{ $item->kategori }}</span>
                                </td>
                                ...
                                <td>
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.news.show', $item->id) }}" class="btn btn-sm btn-ghost text-emerald-600">
                                            👁 Detail
                                        </a>
                                        <a href="{{ route('admin.news.edit', $item->id) }}" class="btn btn-sm btn-ghost">
                                        ...
                                        <form action="{{ route('admin.news.destroy', $item->id) }}"
                                            ...
                                            <dialog class="modal" :class="{ 'modal-open': open }">
                                                <div class="modal-box"><h3 class="font-bold text-lg">Konfirmasi Hapus</h3><p class="py-4">Hapus berita <strong>{{ $item->judul }}</strong>?</p>
