{{--
    ============================================================
    admin\profil\edit.blade.php — Edit Data Profil Yayasan
    ============================================================
    Halaman form edit profil yayasan (versi satu kartu penuh).
    Data $profil dikirim dari ProfilController.edit() dan form
    dikirim ke ProfilController.update() via route admin.profil.update
    (metode PUT, enctype multipart untuk upload berkas).
    Alur halaman: header + tombol kembali → kartu form dengan
    beberapa seksi: identitas (nama, email, telepon/WA, alamat),
    narasi (sejarah, visi, misi), dan upload berkas (surat legalitas
    & struktur pengurus — keduanya opsional) → tombol Batal &
    Simpan, ditutup skrip JS kecil untuk menampilkan nama file.
--}}
<x-admin-layout>
<div class="bg-gradient-to-b from-base-200 to-base-300 min-h-0">

    {{-- Header halaman: judul dan tombol kembali ke halaman profil --}}
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 pb-12">
        ...
        <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-base-200 overflow-hidden">
            <div class="px-8 py-6">
                {{-- Form utama edit profil yayasan (metode PUT) --}}
                <form action="{{ route('admin.profil.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf @method('PUT')

                    {{-- SEKSI 1: Informasi identitas yayasan (3 kolom sejajar) --}}
                    ...
                    {{-- SEKSI 3: Upload Berkas & Gambar (opsional) --}}
                    <p class="font-bold text-base-content mb-4">Upload Berkas &amp; Gambar</p>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {{-- Kolom: Surat Legalitas (foto SK / akta notaris, opsional) --}}
                        <div class="form-control">
                            ...
                            <div class="relative">
                                {{-- Label upload bergaya drag-drop; nama file tampil di #legal-text --}}
                                ...
                                {{-- Input file tersembunyi; event change ditangani skrip bindFile di bawah --}}
                            ...
                            {{-- Pratinjau berkas legalitas saat ini --}}
                        ...
                        {{-- Kolom: Struktur Pengurus (bagan / foto bersama pengurus, opsional) --}}
                        <div class="form-control">
                            ...
                            <div class="relative">
                                {{-- Label upload bergaya drag-drop; nama file tampil di #struktur-text --}}
                            ...
                            {{-- Pratinjau berkas struktur saat ini --}}
                    ...
                    {{-- Tombol aksi: Batal dan Simpan Perubahan --}}
...
{{-- ------------------------------------------------------------------
    SEKSI JAVASCRIPT: bindFile() menghubungkan input file ke label
    teksnya agar nama file yang dipilih ikut tampil di label upload
    (untuk input legalitas dan struktur pengurus).
------------------------------------------------------------------ --}}
<script>
    // Fungsi umum: saat input file berubah, tampilkan nama filenya di span label
    ...
    // Daftarkan event untuk kedua input upload berkas
