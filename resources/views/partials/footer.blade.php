    ...
    <div class="max-w-7xl mx-auto px-4 py-12">
        ...
        <div class="flex items-center gap-3 mb-10">
            @if($profil && $profil->logo)
                <img src="{{ asset('storage/' . $profil->logo) . '?v=' . now()->timestamp }}" class="w-10 h-10 rounded-lg object-cover flex-shrink-0 ring-2 ring-brand-800" alt="Logo">
            @else
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-x-12 gap-y-10">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <ul class="space-y-3">
                        <li><a href="{{ route('profil') }}" class="text-xs text-brand-400 hover:text-brand-200 transition-colors">📖 Tentang Kami</a></li>
                        <li><a href="{{ url('/#kampanye') }}" class="text-xs text-brand-400 hover:text-brand-200 transition-colors">❤️ Program Donasi</a></li>
                        <li><a href="{{ url('/#program-ota') }}" class="text-xs text-brand-400 hover:text-brand-200 transition-colors">🤝 Orang Tua Asuh</a></li>
                        <li><a href="{{ url('/#berita-kegiatan') }}" class="text-xs text-brand-400 hover:text-brand-200 transition-colors">📰 Berita</a></li>
                    </ul>
                </div>
                <div>
                    <p class="text-xs font-semibold text-brand-300 uppercase tracking-wider mb-3">📞 Kontak</p>
                    <ul class="space-y-3">
                        <li><a href="tel:{{ $profil?->no_telp }}" class="text-xs text-brand-400 hover:text-brand-200 transition-colors">📞 {{ $profil?->no_telp ?? '0812-3456-7890' }}</a></li>
                        <li><a href="mailto:{{ $profil?->email }}" class="text-xs text-brand-400 hover:text-brand-200 transition-colors">✉️ {{ $profil?->email ?? 'info@baitulyatim.or.id' }}</a></li>
                    </ul>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="bg-brand-900/40 rounded-lg p-3 mb-3 border border-brand-800/50">
                    <p class="text-xs text-brand-300 leading-relaxed">{{ $profil?->alamat ?? 'Kp. Babakan Cimenteng Rt 37/07, Ds. Gunung Guruh, Kec. Gunung Guruh - Sukabumi' }}</p>
                </div>
        ...
        <div class="mt-10 pt-5 border-t border-brand-900 text-center">
            <p class="text-xs text-brand-500">&copy; {{ date('Y') }} {{ $profil?->nama_yayasan ?? 'Yayasan Baitul Yatim Sukabumi' }}. Dikelola dengan penuh amanah & transparansi.</p>
        </div>
    </div>
</footer>
